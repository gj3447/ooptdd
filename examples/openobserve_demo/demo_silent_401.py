...
# The shipper gets a derived, immutable snapshot containing the stale credential.
# The process environment remains untouched, and the backend receives all ambient
# input explicitly at this application boundary.
broken_environment = MappingProxyType(
    {**environment, "DEMO_STALE_PASSWORD": "stale-credential-from-last-quarter"}
)
broken_shipper = get_backend(
    "openobserve",
    stream="demo",
...
verifier_backend = get_backend("openobserve", stream="demo", environment=environment)
...
print(
    "ABSENT — the store is the judge: 'shipped OK' lied, and the gate caught it "
