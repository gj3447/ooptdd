...
**Caveat**: Tested only in production BPC cycle (single day); large-scale (1000+ span/s) unvalidated. Langfuse/LangSmith may have equivalent internal retry logic (not public).
...
**ooptdd unique**: `@pytest.fixture def oo_trace(cycle_id, cid)` + `gates/bpc_smoke.yaml` in repo + `conftest` hook. OO_URL is env-var (must be set); if env absent, test gate exits 0 (complete no-op). **Portability**: lakatotree ↔ prismv2 using same oo_sink/oo_verify stubs (only conftest glue differs).

**Caveat**: Requires external OpenObserve instance (or equivalent log store). Smallest deployment is still non-trivial (airo-edge colo).
...
**Rationale**: Langfuse is open-source, ClickHouse is standard OLAP. Replacing OpenObserve would eliminate airo-edge infra risk (single point of failure).
...
**Action**: Low priority. Only if airo-edge OpenObserve becomes liability.
...
**Action**: Proposed in future. Current monolithic ooptdd is more maintainable for single team (BPC).
...
ooptdd was validated in *one cycle* (2026-06-10) with known actors (kjra + Claude agents). Risks not yet observed:
...
`verify_trace()` defaults (strict=14s, warn=4.5s) were tuned for BPC cycle (single agent, 20-view E2E ≤12s). Different agent architectures (multi-threaded, distributed) may require per-cycle tuning. No principled formula.
