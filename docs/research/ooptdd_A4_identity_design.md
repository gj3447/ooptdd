...
5. **License = Apache 2.0 (patent grant matters for Acme robotics IP)**. MEDIUM-HIGH confidence. Rationale: (a) Acme background (automotive/robotics QA) implies latent patent portfolio (consumer_b measurement, HALCON integration, distributed calib); (b) Apache 2.0 includes explicit patent grant (35 USC 271) — critical if someone embeds the methodology into a closed-source agent-safety product; (c) MIT is simpler but offers no patent indemnity (Acme could be sued by fork-user). Downside of Apache: longer legal read. Upside: "we thought about this" signals enterprise readiness. MIT alternative is defensible if Acme disclaims patent intent, but Apache is the safer default for semi-public methodology from hardware company.
...
3. **License: MIT (simplicity over patent signal)**. Rationale: Acme doesn't need patent-grant explicit clause if methodology is open and the company isn't claiming improvements; MIT is 5-liner, Apache is legal reading. Downside: if Acme patents the actual conftest hook orchestration later, MIT forks are unprotected. Verdict: MIT is OK if Acme commits to "methodology stays open-source forever" in governance docs.
...
6. **"Open-sourcing this loses Acme' competitive edge in AI-driven inspection."** Counter: The methodology is the edge, not the code (103LOC oo_sink.py + 122LOC oo_verify.py). Publishing accelerates adoption + feedback + academic citations (KG binding + Longinus proves methodology integrity). Caveat: Acme should protect the *application* (consumer_b measurement, part_375 tolerances, HALCON integration) — which is not open-sourced — by not bundling inspection-specific gates in the OSS project (keep them internal).
...
**License**: Apache 2.0 (patent grant matters for Acme' robotics IP; better safe default for semi-public methodology from hardware company).
