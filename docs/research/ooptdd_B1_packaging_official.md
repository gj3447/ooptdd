...
**Claim:** For ooptdd config (`CONSUMER_LOGS_E2E`, `AIRO_LOGS_VERIFY`, `CONSUMER_A_TEST_CID`), env-vars override all; users can also pass `-o ooptdd_log_level=strict` or set `[tool.pytest.ini_options] ooptdd_log_level = "strict"` in pyproject.toml.
...
**Implication:** Acme-internal users set `CONSUMER_LOGS_E2E=1` in `.env.prod`; external users rely on pyproject.toml.
...
**Confidence:** MEDIUM — Pattern is PEP 508 standard; pytest-cov and pytest-xdist both use this. Acme internal can skip extras (always require OO SDK) but public distribution benefits.
...
**Confidence:** HIGH — Official pytest docs mandate plugin opt-out via `-p no:NAME`. Acme usage: offline CI (drone) or dev-machine without network should disable ooptdd.
...
### Caveat-4: Private Repository Setup (If Acme-Internal Release)
