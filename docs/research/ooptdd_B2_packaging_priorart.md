...
Wrap pytest invocation in a shell script that sets env vars (e.g., `CONSUMER_LOGS_E2E=1`), then calls pytest with `-p pytest_ooptdd`. Pros: zero config for users; Cons: fragile, OS-specific, hard to compose with other tools.
...
**Current state**: `tests/_oo_ltdd/conftest.py` + `_oo_ltdd/` module; opt-in via `CONSUMER_LOGS_E2E=1`.
