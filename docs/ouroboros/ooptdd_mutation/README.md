# OOPTDD mutation profile and meta-audit case study

This document describes an explicit, domain-specific Ouroboros profile. It is not part
of the generic kernel contract or the base wheel. Install the first-party distribution
under `extensions/ooptdd-mutation/`, then import `ooptdd_mutation.ouroboros` only when
this workflow is wanted.

[`ooptdd-mutation mutate`](../../ci_mutation_gate.md) answers "would my gate notice a broken
...
## Relation to `ooptdd-mutation mutate`

Same epistemology, different direction. `ooptdd-mutation mutate` plants failures in the
...
The executable API is namespaced under
`ooptdd_mutation.ouroboros`.
...
Authenticated artifact completion is a separate, explicit policy.  A frozen
`AuthenticatedGateCompletionPolicy` contains an ordered tuple of role/artifact contracts,
the claimed authority identity, and independent authentication and independence requirements.
There is no implicit role set or trust default.  Call
`ooptdd_mutation_v2_completion_policy(authority_id)` to reproduce this profile's current
four-role v2 gate-evidence behavior; callers may deliberately construct policies selecting
zero, one, three, or four roles when a narrower completion projection is intended.
...
- `module-decision.json` — module-vs-engine ADR and promotion falsifiers.
- `fsm-spec.json` — machine-readable normal-flow and mutation-safety
  projection; reducer tests cover the explicitly scoped fault overlay.
- `fsm-traces.json` — transition, guard-false, and invalid-event traces.
- `fsm.mmd` — generated-view diagram; the JSON remains authoritative.
- `bounded-execution-policy.json` — budgets, effects, and recovery boundary.
- `verification.md` — claim/evidence matrix and remaining gaps.
