...
from ooptdd.reports import RENDERERS, to_junit_xml, to_markdown

CID = "report-cid"


def test_public_renderer_catalog_is_immutable():
    with pytest.raises(TypeError):
        setitem(RENDERERS, "json", to_markdown)


def _res(expect, events, *, reachable=True):
    return gate.evaluate_events(
        {"cid": CID, "expect": expect},
        events,
        reachable=reachable,
        cid=CID,
        emit_backend="MemoryBackend",
        emit_identity="memory:demo",
    )
...
def test_junit_green_and_red_counts():
    res = _res(
        [{"event": "boot", "op": "gte", "target": 1}, {"event": "never", "op": "gte", "target": 1}],
        _events(),
    )
...
def test_label_survives_rule_shaped_matcher_lists():
    ...
    assert _label({"present": [{"event": "a"}, {"event": "b", "where": {"s": 1}}]}) == "present:a,b"
    findings = lint_spec(
        {
            "cid": CID,
            "expect": [
...
def test_cli_gate_report_junit(tmp_path, monkeypatch):
    ...
    with pytest.raises(SystemExit):
        main(["gate", str(spec), "--report", "junit"])
