...
def test_openobserve_pages_to_completion(monkeypatch):
    ...
    b = OpenObserveBackend(
        ...
        opener=_oo_paging_opener(dataset, seen=seen),
    )
...
def test_openobserve_runaway_ceiling_surfaces_incomplete(monkeypatch):
    ...
    b = OpenObserveBackend(
        base_url="http://oo.test:5080",
        environment={"OOPTDD_OO_PASSWORD": "x"},
        opener=_oo_paging_opener(dataset, seen=[]),
        max_rows=1500,
    )
    r = b.query("c1", since_us=0, until_us=10**18)
...
def test_openobserve_cid_is_escaped_not_interpolated(monkeypatch):
    ...
    assert "cid = 'c1'' OR ''1''=''1'" in captured["sql"]
...
def test_openobserve_ship_raises_on_http_error(monkeypatch):
    ...
    b = OpenObserveBackend(
        base_url="http://oo.test:5080",
        environment={"OOPTDD_OO_PASSWORD": "x"},
        opener=lambda req, timeout: _Resp(status=500),
    )
...
def test_victorialogs_cid_escaped_in_filter(monkeypatch):
    ...
    VictoriaLogsBackend(base_url="http://vl.test:9428", opener=opener).query(
        'c1"x', since_us=0, until_us=1
    )
...
def test_victorialogs_ship_raises_on_http_error(monkeypatch):
    ...
    b = VictoriaLogsBackend(
        base_url="http://vl.test:9428", opener=lambda req, timeout: _Resp(status=503)
    )
    with pytest.raises(OSError):
...
def test_clickhouse_limit_and_completeness(monkeypatch):
    ...
    b = ClickHouseBackend(base_url="http://ch.test:8123", opener=opener, max_rows=2)
...
def test_clickhouse_ship_raises_on_http_error(monkeypatch):
    monkeypatch.setenv("OOPTDD_CH_URL", "http://ch.test:8123")
    b = ClickHouseBackend(
        base_url="http://ch.test:8123", opener=lambda req, timeout: _Resp(status=500)
    )
    with pytest.raises(OSError):
        b.ship([{"cid": "c1", "event": "e"}])
