...
def test_where_filter_multi_field_and_semantics():
    ...
    _ship(
        b,
        "c1",
        {"event": "cycle", "verdict": "NG", "station": "A"},
...
def test_openobserve_select_star_passes_through_arbitrary_fields():
    ...
    def opener(req, timeout):
        ...
        return _R(
            json.dumps({"hits": [{"event": "cycle", "verdict": "NG", "_timestamp": 1}]}).encode()
        )

    b = OpenObserveBackend(
        base_url="http://oo.test:5080",
        environment={"OOPTDD_OO_PASSWORD": "x"},
        opener=opener,
    )
...
def test_absent_fails_and_surfaces_offender():
    ...
    _ship(
        b,
        "c1",
        {"event": "cycle.final_verdict", "verdict": "PASS"},
...
def test_captured_env_forbid_errors_injects_default_gate():
    b = MemoryBackend()
    _ship(
        b,
        "c1",
    ...
    res = evaluate(
        b,
        {"cid": "c1", "expect": [{"present": [{"event": "cycle.final_verdict"}]}]},
...
def test_captured_env_forbid_errors_green_when_clean():
    b = MemoryBackend()
    _ship(b, "c1", {"event": "cycle.final_verdict", "verdict": "PASS"})
    res = evaluate(
        b,
...
def test_allow_errors_allowlists_a_known_benign_error():
    b = MemoryBackend()
    _ship(
        b,
        "c1",
...
def test_spec_forbid_errors_false_opts_out_of_captured_env_default():
    b = MemoryBackend()
    _ship(
        b,
        "c1",
