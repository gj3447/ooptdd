...
def test_captured_env_on_blocks_a_single_authority_gate():
    res = _eval(
        _SELF,
        [{"event": "a", "_timestamp": 1}],
        environ={"OOPTDD_REQUIRE_CORROBORATION": "1"},
    )
    assert res["ok"] is False and res["uncorroborated"] is True
    assert res["oracle"]["enforced"] is True


def test_spec_flag_overrides_captured_env():
    res = _eval(
        {**_SELF, "require_corroboration": False},
        [{"event": "a", "_timestamp": 1}],
        environ={"OOPTDD_REQUIRE_CORROBORATION": "1"},
    )
    assert res["ok"] is True and res["uncorroborated"] is False
...
def test_separate_source_external_satisfies():
    res = _eval(
        _EXT,
        [],
        probe=_Probe(42, separate_source=True),
        environ={"OOPTDD_REQUIRE_CORROBORATION": "1"},
    )
...
def test_non_separate_source_probe_is_refused_as_corroboration():
    res = _eval(
        _EXT,
        [],
        probe=_Probe(42, separate_source=False),
        environ={"OOPTDD_REQUIRE_CORROBORATION": "1"},
    )
    assert res["ok"] is False and res["uncorroborated"] is True
...
def test_unreachable_probe_is_inconclusive_not_uncorroborated():
    class _Unreachable:
        def probe(self, kind, selector, cid):
...
def test_same_endpoint_demotes_a_declared_separate_source():
    res = _eval(
        _EXT,
        [],
        probe=_Probe(42, separate_source=True, derived_identity="https://store-a"),
...
def test_distinct_endpoint_keeps_corroboration():
    res = _eval(
        _EXT,
        [],
        probe=_Probe(42, separate_source=True, derived_identity="https://store-b"),
        emit_identity="https://store-a",
        environ={"OOPTDD_REQUIRE_CORROBORATION": "1"},
    )
    assert res["ok"] is True
...
def test_demotion_is_asymmetric_no_emit_identity_never_demotes():
...
def test_oracle_surfaces_emit_provenance():
    res = _eval(
        _SELF,
        [{"event": "a", "_timestamp": 1}],
        emit_backend="MemoryBackend",
        emit_identity="memory://local",
    )
