...
_POLICY = AuthenticatedGateCompletionPolicy("authority://test")
...
def _receipt_and_artifacts(
    *, duplicate_artifact: bool = False, context_mismatch: bool = False
):
...
def test_unresolved_gate_artifact_fails_while_legacy_receipt_validation_stays_unchanged():
    receipt, artifacts = _receipt_and_artifacts()
...
def test_duplicate_artifact_across_roles_is_rejected_before_resolution():
    ...
    assert not result.satisfied
    assert result.errors == ("the four run roles must bind four unique gate artifacts",)
    assert resolver.calls == []
...
def test_public_completion_values_reject_malformed_or_contradictory_states():
    ...
    with pytest.raises(ValueError, match="requires four validated artifacts"):
        CompletionAssessment(_POLICY, True, (), ())
    wrong_type = tuple(_digest(f"wrong-type-{index}") for index in range(4))
    with pytest.raises(ValueError, match="typed Ouroboros gate artifacts"):
...
def test_local_assessment_treats_malformed_resolved_mapping_as_failure_not_crash():
    ...
    result = assess_authenticated_gate_completion(
        receipt, malformed, policy=_POLICY  # type: ignore[arg-type]
    )
