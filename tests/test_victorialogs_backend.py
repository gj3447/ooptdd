...
Ship emits JSON-lines to /insert/jsonline with cid as a stream field;
...
def test_ship_emits_jsonlines_with_stream_field(monkeypatch):
    ...
    b = VictoriaLogsBackend(base_url="http://vl.test:9428", opener=opener)
    b.ship([{"event": "cycle", "cid": "c1"}, {"event": "cycle", "cid": "c1", "verdict": "PASS"}])
    assert "/insert/jsonline" in captured["url"]
    assert "_stream_fields=cid" in captured["url"] and "_msg_field=event" in captured["url"]
...
def test_query_builds_logsql_filter_and_window(monkeypatch):
    ...
    def opener(req, timeout):
        captured["url"] = req.full_url
        row = {"event": "cycle", "cid": "c1", "verdict": "NG", "_time": "2026-06-18T00:00:00Z"}
    ...
    assert "cid" in captured["url"] and "c1" in captured["url"]
...
def test_query_marks_malformed_jsonline_as_incomplete(monkeypatch):
    monkeypatch.setenv("OOPTDD_VL_URL", "http://vl.test:9428")
    body = b'{"event":"a","cid":"c1"}\nnot-json-should-be-skipped\n{"event":"b","cid":"c1"}\n'
    ...
    assert r.reachable and [e["event"] for e in r.events] == ["a"]
    assert r.complete is False
    assert r.error == "ValueError: VictoriaLogs response contains malformed JSON"
...
def test_gate_evaluates_over_victorialogs_rows(monkeypatch):
    ...
    monkeypatch.setenv("OOPTDD_VL_URL", "http://vl.test:9428")
    body = b'{"event":"cycle.final_verdict","cid":"c1","verdict":"PASS"}\n'
    b = VictoriaLogsBackend(base_url="http://vl.test:9428", opener=lambda req, timeout: _Resp(body))
