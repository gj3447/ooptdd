...
from ooptdd.mutation import mutation_report, ndcg, ranked_kills, verify_audit_ranking
...
def test_ranked_kills_orders_kills_first_ties_by_canonical_position():
    report = {"mutations": [
        {"mutation_id": "s0", "mutation": "m0", "operator": "op", "status": "survived",
         "caught": False},
        {"mutation_id": "k1", "mutation": "m1", "operator": "op", "status": "killed",
         "caught": True},
        {"mutation_id": "k2", "mutation": "m2", "operator": "op", "status": "killed",
         "caught": True},
    ]}
    view = ranked_kills(report)
    assert [(r["rank"], r["mutation_id"], r["relevance"]) for r in view] == [
        (1, "k1", 1), (2, "k2", 1), (3, "s0", 0)]
...
def test_verify_refuses_a_ranking_that_is_not_a_permutation():
    ...
    foreign = verify_audit_ranking(report, MIXED_SPEC, MIXED_EVENTS,
                                   [*ids[:-1], "0000000000000000"])
...
def test_cli_permuted_ranking_is_the_measured_red_then_restores(tmp_path, capsys):
    ...
    assert main(["audit-rank", str(spec), "--events", str(ev), "--report", str(rep),
                 "--ranking", str(ranking)]) == 0
...
def test_cli_refuses_an_all_survivor_report(tmp_path, capsys):
    spec = tmp_path / "gate.yaml"
    spec.write_bytes(b"cid: mut-rank\n"
                     b"expect:\n"
                     b"  - tool_calls: {expected: [a, b], match: subset, target: 0.5}\n")
    ev = tmp_path / "events.json"
...
def test_cli_refuses_unmeasured_and_baseline_red_reports(tmp_path, capsys):
    spec, ev, rep, report = _materialize(tmp_path)
    rep.write_bytes(json.dumps({"baseline_green": True, "canary_survived": False,
                                "score_status": "unmeasured", "mutations": []}).encode())
...
def test_cli_lock_pins_the_spec_sha(tmp_path, capsys):
    ...
    from ooptdd.mutation import LOCK_SCHEMA
    spec, ev, rep, _ = _materialize(tmp_path)
    lock = tmp_path / "lock.json"
    lock.write_bytes(json.dumps({
        "schema": LOCK_SCHEMA, "min_score": 0.8,
