...
class LaggyBackend:
    ...
    def __init__(self, clock, events, visible_after_us, visibility_ms):
        ...
        self.caps = BackendCaps(
            queryable=True, supports_where=True, query_visibility_delay_ms=visibility_ms
        )
        self.flushes = 0
    ...
    def query(self, cid, *, since_us, until_us):
        ...
        return QueryResult(reachable=True, events=[e for e in visible if e.get("cid") == cid])
...
def test_lagged_arrival_inside_blind_window_is_not_absent():
    ...
    res = verify_gate(backend, "c", SPEC, retries=1, delay=0.1, clock=clock, sleeper=sleeper)
...
def test_truly_absent_past_the_window_is_still_red():
    ...
    res = verify_gate(backend, "c", SPEC, retries=3, delay=1.0, clock=clock, sleeper=sleeper)
...
def test_memory_backend_declares_zero_window_no_behavior_change():
    ...
    backend.ship([{"cid": "m", "event": "a"}])
    clock = FakeClock()
    sleeper = AdvancingSleeper(clock)
    res = verify_gate(backend, "m", SPEC, retries=1, delay=0.1, clock=clock, sleeper=sleeper)
...
def test_victorialogs_force_flush_posts_the_documented_endpoint():
    seen = {}
    ...
    vl = VictoriaLogsBackend(base_url="http://vl:9428", opener=opener)
