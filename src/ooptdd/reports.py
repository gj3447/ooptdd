...
def to_junit_xml(result: dict, *, suite: str = "ooptdd.gate") -> str:
    ...
    optional misses are <skipped> too (surfaced, never red)."""
    infra = _infra(result)
    ...
    for label, chk, detail in _check_rows(result):
        ...
        if infra is not None:
            ...
            if chk.get("optional"):
                skipped += 1
                body = f"<skipped message={quoteattr('optional check missed')}>{payload}</skipped>"
            else:
                failures += 1
...
def to_markdown(result: dict) -> str:
    ...
    for label, chk, detail in _check_rows(result):
        if infra is not None:
            ...
            state = "⏭ optional-miss" if chk.get("optional") else "❌ fail"
