...
import pytest
from ooptdd import check, checks_from, compose_check_registry, evidence_tier
...
def test_adapter_payload_is_accepted_by_the_existing_v2_reducer() -> None:
    materials = _materials()
    snapshot = CycleSnapshot.start("cycle-0", ProtocolBudget(max_steps=6, max_generations=1))
...
def test_adapter_drives_all_four_roles_into_a_valid_complete_receipt() -> None:
    ...
    snapshot = CycleSnapshot.start("cycle-0", ProtocolBudget(max_steps=16, max_generations=1))
    ...
    assert [run["artifact"] for run in receipt["runs"]] == [
        bundle.artifact.to_dict() for bundle in (initial, positive, negative, regreen)
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda doc: doc["oracle"].__setitem__("evidence_tier", "queryable_causal"),
