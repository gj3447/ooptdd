...
import ooptdd_mutation.benchmarks.tier0 as benchmark_module
from ooptdd_mutation.benchmarks.evidence_integrity import (
    EvidenceIntegrityError,
    measurement_environment,
    prospective_git_receipt,
)
from ooptdd_mutation.benchmarks.tier0 import (
    canonical_json,
...
FIXTURE_DIR = benchmark_module.DEFAULT_FIXTURE_DIR
...
def efficacy_bundle(tmp_path_factory: pytest.TempPathFactory) -> EfficacyBundle:
    ...
    _write(
        preregistration,
        {
            ...
            "sources": {"registration_repository": str(governance_origin)},
            "prediction": {
...
def _published_governance_branch(
    ...
    *,
    ...
    preregistration = governance / bundle.preregistration.relative_to(bundle.governance_root)
    if lock_updates:
...
def test_measurement_clean_e2e_has_exact_negative_and_canonical_projections(
    ...
    assert prospective["preregistration"]["sha256"] == _sha256(efficacy_bundle.preregistration)
    assert prospective["measurement_lock"]["sha256"] == _sha256(efficacy_bundle.lock)
    assert prospective["preregistration"]["commit"] != prospective["measurement_lock"]["commit"]
    ...
    assert [row["id"] for row in negative["scenarios"] if row["oracle_match_rate"] != 1.0] == [
        "late-offender-confirm"
    ]

    deepeval = sequence["deepeval"]
    candidate = _read(efficacy_bundle.measurement_dir / deepeval["candidate"]["file"])
    injected = _read(efficacy_bundle.measurement_dir / deepeval["injected_mismatch"]["file"])
