...
def _rows(*, lag_false_reds: int = 0, outage_honest: bool = True, restored: bool = True):
    def loss_samples():
        return [{"observed": "absent", "oracle_match": True} for _ in range(3)]
...
def test_binding_violations_are_pure_and_fail_loud():
    ...
    assert (
        ...
        in T1.binding_violations(lock, **{**good, "prereg_sha256": "d" * 64})[0]
    )
...
def test_preflight_mismatches_name_the_drifted_hash():
    ...
    assert (
        T1.preflight_binding_mismatches(lock, provenance=provenance, deepeval_spec_sha256="7" * 64)
        == {}
    )
