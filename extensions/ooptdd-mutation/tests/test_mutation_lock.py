...
from ooptdd_mutation.analysis import LOCK_SCHEMA, verify_mutation_lock
...
def test_unknown_schema_and_missing_fields_are_refused():
    ...
    assert (
        verify_mutation_lock(SPEC_TEXT.encode(), {"schema": LOCK_SCHEMA, "min_score": "high"})[0]
        is None
    )
...
def test_cli_refuses_threshold_repick_after_lock(tmp_path, capsys):
    spec, ev, lock = _materialize(tmp_path, min_score=0.8)
    rc = main(["mutate", str(spec), "--events", str(ev), "--lock", str(lock), "--min-score", "0.1"])
