...
from ooptdd_mutation.analysis import derive_mutations, mutation_report, ooptdd_checks
...
_EXTENSION_SOURCE = Path(__file__).resolve().parents[1] / "src"
_TRAJECTORY_SOURCE = Path(__file__).resolve().parents[2] / "ooptdd-trajectory" / "src"
...
def test_existence_only_gate_has_a_blind_spot():
    ...
    spec = {
        "expect": [
            {"present": [{"event": "cycle", "where": {"verdict": "PASS"}}]},
...
def test_mutation_policy_is_explicit_and_does_not_read_environment(monkeypatch):
    monkeypatch.setenv("OOPTDD_FORBID_ERRORS", "true")
    spec = {"expect": [{"present": [{"event": "request.end"}]}]}

    assert "inject_error" not in {name for name, _ in derive_mutations(_EVENTS, spec)}
    explicit = GatePolicy(forbid_errors=True)
    report = mutation_report(_EVENTS, spec, policy=explicit)
    assert "inject_error" in {row["mutation"] for row in report["mutations"]}


def test_mutation_report_uses_injected_event_judge():
    calls = []

    def judge(spec, events, **context):
        calls.append((events, context))
        return {"ok": bool(events)}

    report = mutation_report(_EVENTS, {"expect": []}, judge=judge)

    assert report["baseline_green"] is True
    assert report["canary_survived"] is False
    assert all(call[1]["policy"] == GatePolicy() for call in calls)


def test_mutation_report_freezes_one_registry_snapshot_for_every_judgement():
    registries = []

    def judge(spec, events, **context):
        registries.append(context["registry"])
        return {"ok": bool(events)}

    mutation_report(
        _EVENTS,
        {"expect": [{"present": [{"event": "request.end"}]}]},
        judge=judge,
    )

    assert len(registries) >= 3  # baseline, at least one mutant, and canary
    assert all(registry is registries[0] for registry in registries)
    assert "tool_calls" in registries[0]


def test_unknown_predicate_is_refused_before_baseline_evaluation():
    calls = []
    ...
    with pytest.raises(ValueError, match="unknown gate predicate.*tool_callz"):
        mutation_report(
            _EVENTS,
            {"expect": [{"tool_callz": {"expected": ["search"]}}]},
            judge=judge,
        )
    assert calls == []


def test_mutation_extension_import_is_inert_and_provider_is_explicit():
    code = f"""
import json
import sys
sys.path.insert(0, {str(_TRAJECTORY_SOURCE)!r})
sys.path.insert(0, {str(_EXTENSION_SOURCE)!r})
from ooptdd import compose_check_registry
before = sorted(compose_check_registry())
import ooptdd_mutation as mutation
after = sorted(compose_check_registry())
provided = sorted(mutation.ooptdd_checks())
print(json.dumps({{"before": before, "after": after, "provided": provided}}))
"""
    ...
    assert "tool_calls" not in state["before"]
    assert state["after"] == state["before"]
    assert "tool_calls" in state["provided"]


def test_mutation_provider_composes_through_the_named_runtime_api():
    runtime = compose_runtime(
        project={"extensions": ["ooptdd-mutation"]},
        environment={},
        extension_providers={"ooptdd-mutation": ooptdd_checks},
    ).activate_extensions()

    assert "tool_calls" in runtime.check_registry
    assert runtime.settings.extensions == ("ooptdd-mutation",)
...
def test_tool_call_trajectory_mutations_are_non_vacuous_and_killed():
    ...
    assert all(row["mutation_id"] and row["status"] == "killed" for row in rep["mutations"])
...
def test_ordered_three_call_gate_mutates_every_adjacent_pair():
    events = [
        {"event": "gen_ai.execute_tool", "gen_ai.tool.name": name} for name in ("a", "b", "c")
    ]
    spec = {
        "expect": [
            {
...
def test_tolerant_trajectory_threshold_surfaces_a_surviving_required_tool_mutant():
    ...
    assert set(rep["survivors"]) == {
        "rename_required_tool:0:a",
        "rename_required_tool:1:b",
    }
