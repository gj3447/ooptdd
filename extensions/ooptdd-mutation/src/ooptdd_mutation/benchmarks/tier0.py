"""Deterministic, mechanics-only mutation-arrival benchmark.
...
DEFAULT_FIXTURE_DIR = Path(__file__).with_name("arrival") / "v0"


def _fixture_root(fixture_dir: Path | None):
    return DEFAULT_FIXTURE_DIR if fixture_dir is None else Path(fixture_dir)


def _load_fixture_gate(path) -> dict:
    return load_gate(str(path))
...
def _code_manifest() -> dict[str, str]:
    """Identity of the framework plus this repository-only benchmark harness."""
    package = resources.files("ooptdd")
    manifest = {
        f"ooptdd/{'/'.join(parts)}": sha256_file(resource)
        for parts, resource in _python_resources(package)
    }
    manifest["benchmarks/tier0.py"] = sha256_file(Path(__file__))
    manifest["benchmarks/evidence_integrity.py"] = sha256_file(
...
def _run_mutation_sample(
    ...
    *,
    ...
    events = json.loads(fixture_dir.joinpath("trajectory-events.json").read_text(encoding="utf-8"))
...
def _metrics(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "M2b_late_offender_catch_rate": {
            ...
            "target": 1.0,
            "control_miss_rate": _rate(control, lambda sample: sample["observed"] == "present"),
        },
        "M3_inconclusive_honesty_rate": {
            "value": _rate(
                outage,
                lambda sample: (
                    sample["observed"] == "inconclusive" and sample["junit_failures"] == 0
                ),
            ),
            "target": 1.0,
        },
...
def _conformance(rows: list[dict]) -> dict:
    by_id = {row["id"]: row for row in rows}
    verification = [
        sample for row in rows if row["id"] != "trajectory-mutation" for sample in row["samples"]
    ]
    ...
    no_early_absent = all(
        not (
            ...
            and sample["arrival"]["waited_ms"] < sample["arrival"]["visibility_delay_ms"]
        )
...
def _provenance(fixture_dir, manifest: dict) -> dict:
    ...
    hashes = {
        **fixture_hashes,
        ...
        "runner": code_manifest["benchmarks/tier0.py"],
    }
...
def validate_tier0_result(
    ...
    *,
    ...
    rejected before the result can enter an external evidence record.
    """
    ...
    manifest = json.loads(fixture_dir.joinpath("manifest.json").read_text(encoding="utf-8"))
        ...
        for sample in samples:
            ...
            if row.get("id") == "trajectory-mutation":
                measured = (
                    ...
                    and sample.get("canary_survived") is manifest["mutation"]["canary_survived"]
                    ...
                    and len(sample.get("operators") or []) == manifest["mutation"]["eligible"]
                )
