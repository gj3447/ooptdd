"""Validation for benchmark and externally judged evidence.
...
external judge can replay it without importing any evaluation platform.
"""
...
def validate_registration_repository(lock: dict, preregistration: dict, receipt: dict) -> None:
    ...
    preregistered = sources.get("registration_repository") if isinstance(sources, dict) else None
...
def _unique_rows(observations: Any, keys: tuple[str, ...]) -> list[dict]:
    ...
    for index, row in enumerate(observations):
        ...
        if not isinstance(row.get("matched"), bool):
            raise EvidenceIntegrityError(f"observation {identity!r} matched must be boolean")
        rows.append(row)
    return rows
...
def deepeval_metrics(observations: Any) -> dict:
    ...
    for row in rows:
        ...
        derived = (
            float(observed_score) == float(expected_score) and observed_success is expected_success
        )
        if row["matched"] is not derived:
            raise EvidenceIntegrityError(
...
def validate_measurement(
    ...
    *,
    ...
    for key, value in computed.items():
        ...
        if stored[key] != value:
            raise EvidenceIntegrityError(
                f"stored metric {key!r} is not observation-derived: {stored[key]!r} != {value!r}"
            )
...
def validate_deepeval_measurement(
    ...
    *,
    ...
    if environment.get("deepeval") != expected_version:
        raise EvidenceIntegrityError(
            f"DeepEval version mismatch: {environment.get('deepeval')!r} != {expected_version!r}"
        )
    expected_oracles = {
        "safe": (1.0, True),
