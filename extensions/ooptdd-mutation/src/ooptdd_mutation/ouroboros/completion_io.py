...
def resolve_and_assess_authenticated_gate_completion(
    ...
    *,
    ...
    for artifact in artifacts:
        ...
        if payload is None:
            continue
        binding_errors = validate_gate_artifact_binding(receipt, artifact, payload, policy=policy)
        if binding_errors:
    ...
    assessment = assess_authenticated_gate_completion(receipt, resolved, policy=policy)
    if not shell_errors:
        return assessment
