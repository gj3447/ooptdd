"""Receipt v2 construction and semantic replay."""
...
from ooptdd.identity import CANONICALIZATION, MAX_INTEROPERABLE_INTEGER
...
OUROBOROS_RECEIPT_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "status": {"enum": ["complete", "superseded"]},
...
def _validate_integrity(receipt: dict[str, Any], errors: list[str]) -> None:
...
def validate_receipt(receipt: Any) -> list[str]:
    ...
    if isinstance(status, str) and status in {"complete", "superseded"}:
        ...
        errors.append("status must be complete or superseded")
    ...
    _validate_integrity(receipt, errors)
    if isinstance(status, str) and status in {"complete", "superseded"}:
