...
_NONNEGATIVE_DECIMAL_RE = re.compile(r"^(?:0|[1-9][0-9]*)(?:\.[0-9]+)?(?:e[+-]?[0-9]+)?$")
...
OUROBOROS_GATE_EVIDENCE_SCHEMA: dict[str, Any] = {
    ...
    "$id": ("https://github.com/gj3447/ooptdd/schema/ouroboros-gate-evidence-v1.schema.json"),
        ...
        "oracle": _closed_schema(
            {
                ...
                "emit_backend": _TEXT,
                "emit_identity": _TEXT,
                "emit_independent": {"oneOf": [{"type": "boolean"}, {"type": "null"}]},
                "read_identities": {
                    "type": "array",
                    "uniqueItems": True,
                    "items": _TEXT,
                ...
                "authenticated": {"oneOf": [{"type": "boolean"}, {"type": "null"}]},
...
def _projected_weighted_score(checks: list[dict[str, Any]]) -> float:
    ...
    passed_weight = sum(
        weight for check, weight in zip(checks, weights, strict=True) if check.get("passed") is True
    )
...
def _projected_evidence_tier(
    ...
    *,
    ...
    if any(check["passed"] and check["grounding"] == "corroborated" for check in gating):
...
def _project_arrival(value: Any) -> dict[str, Any]:
    ...
    if set(arrival) != required:
        raise ValueError("verification.arrival fields must be exactly " + repr(sorted(required)))
    return {
        ...
        "confirm_rounds_run": _integer(arrival["confirm_rounds_run"], "arrival.confirm_rounds_run"),
    }
...
def _project_checks(checks: Any) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for ordinal, raw in enumerate(checks):
        ...
        if monitor is not None and (
            not isinstance(monitor, str) or monitor not in {item.value for item in MonitorVerdict}
        ):
        ...
        item = {
            ...
            "gating": gating,
            "weight": _nonnegative_number_text(check.get("weight"), f"checks[{ordinal}].weight"),
        ...
        if not isinstance(item["grounding"], str) or item["grounding"] not in {
            ...
            raise ValueError(f"checks[{ordinal}].grounding must be derived-self or corroborated")
...
def adapt_gate_verification(
    *,
    ...
    if subject_outcome is not _EXPECTED_OUTCOME[role]:
        raise ValueError(f"{role.value} requires subject_outcome {_EXPECTED_OUTCOME[role].value!r}")
    ...
    if threshold is None:
        ...
        threshold_text = _positive_unit_number_text(threshold, "verification.gate.threshold")
    ...
    emit_backend = _text(gate_oracle.get("emit_backend"), "verification.gate.oracle.emit_backend")
    emit_identity = _text(
    ...
    if emit_independent is not None:
        emit_independent = _boolean(emit_independent, "verification.gate.oracle.emit_independent")
    corroborated_checks = [
        (raw, projected)
        for raw, projected in zip(gate["checks"], checks, strict=True)
        if projected["gating"] and projected["passed"] and projected["grounding"] == "corroborated"
    ]
    read_identities = sorted(
        {
            raw["derived_identity"]
            for raw, _ in corroborated_checks
            if isinstance(raw.get("derived_identity"), str) and raw["derived_identity"].strip()
        }
    )
    ...
    oracle_gating = _integer(gate_oracle.get("gating"), "verification.gate.oracle.gating")
    ...
    lifecycle_outcome = RunOutcome.INCONCLUSIVE if infra_uncertain else subject_outcome
    ...
    if lifecycle_outcome is not RunOutcome.INCONCLUSIVE and role in {
        RunRole.POSITIVE,
        RunRole.REGREEN,
    }:
...
def validate_gate_evidence(
    ...
    *,
    ...
    aggregation = _exact(root.get("aggregation"), _AGGREGATION_FIELDS, "aggregation", errors)
    ...
    for ordinal, raw in enumerate(checks):
        ...
        if not _valid_nonnegative_decimal(check.get("weight")):
            errors.append(f"checks[{ordinal}].weight must be canonical non-negative decimal text")
        grounding = check.get("grounding")
    ...
    if mode == "all":
        ...
        if not _valid_unit_decimal(aggregation.get("score")) or not _valid_positive_unit_decimal(
            aggregation.get("threshold")
        ):
            errors.append("weighted_threshold requires score in [0, 1] and threshold in (0, 1]")
        elif all(_valid_nonnegative_decimal(check.get("weight")) for check in gating_checks):
            try:
                expected_score = _projected_weighted_score(gating_checks)
    ...
    corroborated = sum(
        1
        for check in checks
        if check.get("gating") and check.get("passed") and check.get("grounding") == "corroborated"
    )
    ...
    signature_evaluated = oracle_doc.get("signature_enforced") is True and bool(gating_ids)
    ...
    uncorroborated = (
        oracle_doc.get("corroboration_enforced") is True and bool(gating_ids) and corroborated == 0
    )
    ...
    expected_verdict = (
        "inconclusive" if infra else "present" if aggregation.get("ok") is True else "absent"
    )
    if verdict != expected_verdict:
        errors.append("readback.verdict does not match reachability and aggregation")
    if not infra and subject is not None:
        expected_subject = RunOutcome.GREEN if aggregation.get("ok") is True else RunOutcome.RED
