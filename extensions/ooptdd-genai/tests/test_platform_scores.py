...
def test_langfuse_poster_hits_scores_endpoint_with_basic_auth():
    ...
    status = post_langfuse_score(
        "https://cloud.langfuse.com", "pk", "sk", langfuse_score_body(GREEN), opener=opener
    )
    assert status == 200
...
def test_phoenix_poster_hits_trace_annotations():
    ...
    status = post_phoenix_annotations(
        "http://phx:6006", phoenix_annotation_payload(RED), api_key="k1", opener=opener
    )
    assert status == 200
