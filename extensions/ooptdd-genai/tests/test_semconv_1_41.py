"""Tests for independently pinned OpenTelemetry GenAI vocabularies."""

from __future__ import annotations

from ooptdd_genai.semconv import (
    _REQUIRED,
    GEN_AI_OPERATIONS,
...
def test_legacy_preset_vocabulary_is_frozen():
    ...
    assert gen_ai_ontology() is not None


# ── track 2: gen_ai@1.41 ───────────────────────────────────────────────────────
def test_1_41_preset_resolves_and_has_nine_operations():
    onto = gen_ai_ontology_1_41()
...
def test_1_41_execute_tool_still_requires_tool_name():
    onto = gen_ai_ontology_1_41()
    res = _violations(onto, {"event": "gen_ai.execute_tool", "gen_ai.provider.name": "anthropic"})
...
def test_1_41_embeddings_requires_provider_not_model():
    ...
    ok = _violations(onto, {"event": "gen_ai.embeddings", "gen_ai.provider.name": "openai"})
    assert ok["passed"]
    # provider.name missing -> RED
    bad = _violations(
        onto, {"event": "gen_ai.embeddings", "gen_ai.request.model": "text-embedding-3"}
    )
...
def test_1_41_bad_provider_enum_value_is_red():
    ...
    bad = _violations(
        onto,
        {"event": "gen_ai.chat", "gen_ai.provider.name": "xai", "gen_ai.request.model": "grok-4"},
    )
