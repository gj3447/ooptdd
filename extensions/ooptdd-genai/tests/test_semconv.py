...
from ooptdd_genai.semconv import GEN_AI_PROVIDERS, SEMCONV_VERSION, gen_ai_ontology


def test_extension_factory_builds_gen_ai_ontology():
    ont = gen_ai_ontology()
    assert ont.get("gen_ai.execute_tool") is not None
    assert "execute_tool" in [n.split(".")[-1] for n in ont.types]


def test_extension_import_does_not_create_a_global_preset_registry():
    assert not hasattr(Ontology, "builtin")
    assert not hasattr(Ontology, "register_preset")
...
def test_execute_tool_conforms_with_tool_name():
    ...
    res = check_conformance(
        ...
        ont,
        event_type="gen_ai.execute_tool",
    )
    assert res["passed"]
...
def test_chat_provider_enum_includes_anthropic_and_rejects_typo():
    ...
    bad = check_conformance(
        [
            {
                "event": "gen_ai.chat",
                "gen_ai.provider.name": "anthropics",  # typo
                "gen_ai.request.model": "claude-opus-4-8",
            }
        ],
        ont,
...
def test_gate_conforms_against_gen_ai_preset():
    ...
    res = evaluate(
        b,
        {"cid": "c1", "expect": [{"conforms": "gen_ai.execute_tool"}]},
