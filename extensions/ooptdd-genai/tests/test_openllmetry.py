"""Tests for the opt-in OpenLLMetry-to-GenAI event adapter."""
...
from ooptdd_genai.openllmetry import (
...
def _tool_span(**attrs):
    base = {
        TRACELOOP_SPAN_KIND: "tool",
        TRACELOOP_ENTITY_NAME: "search_web",
...
def test_provider_value_already_renamed_is_left_alone():
    ev = span_to_event(
        _llm_span(**{"gen_ai.system": None, "gen_ai.provider.name": "openai"}), cid="c"
    )
...
def test_unmappable_span_kind_is_skipped_not_guessed():
    workflow = {
        "name": "wf",
        "attributes": {TRACELOOP_SPAN_KIND: "workflow", TRACELOOP_ENTITY_NAME: "pipeline"},
    }
...
def test_spans_to_events_filters_and_preserves_order():
    spans = [
        _llm_span(),
        {"name": "x", "attributes": {TRACELOOP_SPAN_KIND: "banana"}},
        _tool_span(),
    ]
...
def test_agent_span_maps_with_entity_name_as_agent_id():
    span = {
        "name": "a",
        "attributes": {TRACELOOP_SPAN_KIND: "agent", TRACELOOP_ENTITY_NAME: "planner"},
    }
...
def test_bridge_deeply_snapshots_nested_attributes():
    payload = {"items": [1]}
    event = span_to_event(_tool_span(payload=payload), cid="c")
...
def test_bridge_rejects_malformed_attributes():
    with pytest.raises(TypeError, match="attributes must be a mapping"):
        span_to_event({"attributes": []}, cid="c")
