...
from ooptdd_genai.integrations import (
    emit_verdict_event,
    make_arrival_metric,
...
def fake_deepeval(monkeypatch):
    ...
    if "deepeval" in sys.modules and not isinstance(sys.modules["deepeval"], types.SimpleNamespace):
    ...
    metric = make_arrival_metric(
        _spec(
            [
                ...
                {"event": "invoice.sent", "op": "gte", "target": 1},
            ]
        ),
        backend=mem,
    )
...
def test_verdict_event_IS_invasive_to_a_pin_service_gate_honest(mem):
    ...
    mem.ship(
        [{"event": "boot", "cid": CID, "correlation_id": CID, "cycle_id": CID, "service": "myapp"}]
    )
...
def test_verdict_span_attributes_flat_and_infra_word(mem):
    ...
    res = evaluate_events(
        _spec([{"event": "x", "op": "gte", "target": 1}]), [], reachable=False, cid=CID
    )
    ...
    res_red = evaluate_events(
        _spec([{"event": "x", "op": "gte", "target": 1}]), [], reachable=True, cid=CID
    )
