...
Usage::
    ...
    from ooptdd_genai.integrations import make_arrival_metric
...
def make_arrival_metric(
    gate_spec: dict, *, backend=None, threshold: float = 1.0, name: str = "ooptdd arrival"
):
    ...
    try:
        ...
        raise ImportError(
            ...
            "ooptdd itself does not depend on it"
        ) from exc
    ...
    class ArrivalMetric(BaseMetric):
        ...
        def measure(self, test_case, *_args, **_kwargs) -> float:
            ...
            self.reason = (
                ...
                if not failed
                else f"{len(failed)} gating check(s) missed arrival"
            )
            return self.score
